...
try:
    ...
    if token:
        ...
        search_cmd = f'''curl -s "http://localhost:3001/api/users/search?email=ico@astutetech.co.za" \
          -H "Authorization: Bearer {token}"'''
        stdout, stderr = run_command(search_cmd)
        print(f"Search result: {stdout}")
        
        print(f"\n4. Searching with uppercase ICO@ASTUTETECH.CO.ZA...")
        search_cmd2 = f'''curl -s "http://localhost:3001/api/users/search?email=ICO@ASTUTETECH.CO.ZA" \
          -H "Authorization: Bearer {token}"'''
